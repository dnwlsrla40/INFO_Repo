...
대문자로 작성을 해야하고, 단어와 단어는 언더바(_)로 연결한다.
```
...
따라서 후에 기존의 인터페이스에 추상 메서드를 추가하려면 기존의 모든 구현 클래스들이 모두 에러난다.
...
명시적인 구현 클래스 없이 바로 구현 객체를 얻는 방법
...
이렇게 사용하면 RemoteControl 타입의 함수가 재정의 된 Television의 메서드로 실행됨
